...
for row in roster:  # iterates through roster for inputted house
    if row['middle'] != None:
        print(f"{row['first']} {row['middle']} {row['last']}, born {row['birth']}")  # print first, middle, and last name and year
    else:
        print(f"{row['first']} {row['last']}, born {row['birth']}")  # prints first and last name and year