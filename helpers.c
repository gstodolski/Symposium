...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        ...
        {
            float average = 0;
            average = (image[i][j].rgbtRed + image[i][j].rgbtGreen + image[i][j].rgbtBlue) / 3.000;  //dividing by float
...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            if (sepiaRed > 255)  //ensures color component stays at/under 255
            {
                sepiaRed = 255;
            }
            if (sepiaGreen > 255)
...
void reflect(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < (width / 2); j++)  //only needs to iterate through half the width (swaps with other half)
        {
            RGBTRIPLE temp = image[i][j];
            image[i][j] = image[i][width - j - 1];  //swaps image symmetrically
